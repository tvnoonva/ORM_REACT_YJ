...
const UseEffectHook = () => {
  ...
  //ex)신규 사용자가 사용자 목록에 추가되면 감지해서 해당 사용자의 정보로 메일을 보내주는 기능
  useEffect(() => {
    //userList 배열값이 바뀔때마다 실행되는 기능 구현
    console.log("사용자 목록 정보가 변경되었습니다.", userList);
  });

  //매번 컴포넌트가 랜더링될 때 마다 무언가를 해야할 때 시점 캐치
  //매번 컴포넌트가 랜더링 된다는 것은 데이터 바인딩에 의해 특정 데이터 소스가
