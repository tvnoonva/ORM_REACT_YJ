...
router.get("/mychannels", async (req, res, next) => {
  ...
  try {
    //현재 memberId를 임시로 1로 설정
    //실제 data는 jwt token에서 조회
    let memberId = 1;

    const sqlQuery = `SELECT 
    C.channel_id,
    C.category_code,
    CASE WHEN C.category_code = 2 THEN C.channel_name ELSE (SELECT nick_name FROM channel_member WHERE channel_id=C.channel_id AND member_id != ${memberId}) END AS channel_name,
    CASE WHEN C.category_code = 2 THEN IFNULL(C.channel_img_path,'') ELSE (
    (SELECT profile_img_path FROM member WHERE member_id = (SELECT member_id FROM channel_member WHERE channel_id=C.channel_id AND member_id != ${memberId}))
